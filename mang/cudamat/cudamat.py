...
if platform.system() == 'Windows':
    dll_filename = "libcudamat.dll"
else:
    dll_filename = "libcudamat.so"
_cudamat = ct.cdll.LoadLibrary(os.path.join(current_dir, dll_filename))
...
def generate_exception(err_code):
    ...
    if err_code == -1:
        ...
        return CUDAMatException(
            "Operation not supported on transposed matrices.")
    elif err_code == -6:
        return CUDAMatException("")
    elif err_code == -7:
...
class CUDAMatrix(object):
    ...
    def overwrite(self, array, copy_to_device=True):
        ...
        _cudamat.init_from_array(
            self.p_mat, array.ctypes.data_as(ct.POINTER(ct.c_float)),
    ...
    def blockify(source, blocksize, target=None):
        if target is None:
            target = source
        err_code = _cudamat.blockify(source.p_mat, target.p_mat,
                                     ct.c_uint(blocksize))
        if err_code:
            raise generate_exception(err_code)
        return target

    def generate_translations(source, source_w, target_w, off_x, off_y,
                              target=None):
        ...
        err_code = _cudamat.generate_translations_big_var_off(
            source.p_mat, target.p_mat, off_x.p_mat, off_y.p_mat,
            ct.c_uint(source_w), ct.c_uint(target_w), ct.c_uint(num_channels))
    ...
    def asarray(self):
        """Copy the matrix to an ndarray on the CPU and return it."""
    ...
    def slice(self, first_col, last_col):
        ...
        if self.mat.size[0] == 1 or self.mat.size[1] == 1:
            err_code = _cudamat.get_vector_slice(
                self.p_mat, ct.pointer(mat), ct.c_int(first_col),
                ct.c_int(last_col))
        else:
            err_code = _cudamat.get_slice(
                self.p_mat, ct.pointer(mat), ct.c_int(first_col),
                ct.c_int(last_col))
    ...
    def get_row_slice(self, start, end, target=None):
        ...
        err_code = _cudamat.get_row_slice(
    ...
    def set_row_slice(self, start, end, mat):
        ...
        err_code = _cudamat.set_row_slice(
    ...
    def fill_with_randn(self):
        """
        Fill matrix on the GPU with random numbers drawn from the standard
        normal distribution.
        """
    ...
    def sample_bernoulli(self, target=None):
        """
        Sample a bernoulli distribution. Choose 1 with probability given by
        entries of self, 0 otherwise.
        """

        if not target:
            target = self
        err_code = _cudamat.sample_bernoulli(
    ...
    def sample_bernoulli_tanh(self, target=None):
        """
        Sample a bernoulli distribution. Choose 1 with probability given by
        entries of (1+self)/2, -1 otherwise.
        """

        if not target:
            target = self
        err_code = _cudamat.sample_bernoulli_tanh(
            CUDAMatrix.rnd_state_p, self.p_mat, target.p_mat)
        if err_code:
            raise generate_exception(err_code)
    ...
    def sample_poisson(self, target=None):
        """
        Sample a poisson distribution. Choose 1 with probability given by
        entries of self. Not implemented yet.
        """

        if not target:
            target = self
        err_code = _cudamat.sample_poisson(
            CUDAMatrix.rnd_state_p, self.p_mat, target.p_mat)
        if err_code:
            raise generate_exception(err_code)
    ...
    def sample_gaussian(self, mult=1.0, target=None):
        ...
        if not target:
            target = self
        err_code = _cudamat.sample_gaussian(
    ...
    def perturb_energy_for_softmax_sampling(self, target=None):
        ...
        if not target:
            target = self
        err_code = _cudamat.perturb_energy(
            CUDAMatrix.rnd_state_p, self.p_mat, target.p_mat)
        if err_code:
            raise generate_exception(err_code)
    ...
    def perturb_prob_for_softmax_sampling(self, target=None):
        ...
        if not target:
            target = self
        err_code = _cudamat.perturb_prob(
            CUDAMatrix.rnd_state_p, self.p_mat, target.p_mat)
        if err_code:
            raise generate_exception(err_code)
    ...
    def add_col_mult(self, vec, mult, target=None):
        ...
        err_code = _cudamat.add_col_mult(
    ...
    def mult_diagonal(self, val, target=None):
        ...
        if isinstance(val, CUDAMatrix):
            err_code = \
                _cudamat.mult_diagonal(self.p_mat, val.p_mat, target.p_mat)
        elif isinstance(val, (int, float)):
            err_code = _cudamat.mult_diagonal_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
    ...
    def add_diagonal(self, val, target=None):
        ...
        if isinstance(val, CUDAMatrix):
            err_code = \
                _cudamat.add_diagonal(self.p_mat, val.p_mat, target.p_mat)
        elif isinstance(val, (int, float)):
            err_code = _cudamat.add_diagonal_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix, int or float.")
    ...
    def add_row_mult(self, vec, mult, target=None):
        ...
        err_code = _cudamat.add_row_mult(
            self.p_mat, vec.p_mat, target.p_mat, ct.c_float(mult))
        if err_code:
            raise generate_exception(err_code)
    ...
    def mult_by_col(self, vec, target=None):
        ...
        err_code = \
            _cudamat.mult_by_col_vec(self.p_mat, vec.p_mat, target.p_mat)
        if err_code:
            raise generate_exception(err_code)
    ...
    def mult_by_row(self, vec, target=None):
        ...
        err_code = \
            _cudamat.mult_by_row_vec(self.p_mat, vec.p_mat, target.p_mat)
        if err_code:
            raise generate_exception(err_code)
    ...
    def sum(self, axis=None, target=None):
        """
        Sum the matrix along the given dimension, where 0 represents the
        leading dimension and 1 represents the non-leading dimension.
        If None, the sum of all elements is returned.
        If a target is not provided, a new vector is created for storing
        the result.
        """

        if axis is None:
            return vdot(
                self, CUDAMatrix.ones.slice(0, self.shape[0] * self.shape[1]))
        else:
    ...
    def less_than_eq(self, val, target=None):
        """
        Perform the operation target = 1. * (self < val), where val can be a
        matrix or a scalar.
        """
        ...
        if isinstance(val, (int, float)):
            err_code = _cudamat.less_than_eq_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            err_code = _cudamat.less_than_eq(
    ...
    def less_than(self, val, target=None):
        ...
        if isinstance(val, (int, float)):
            err_code = _cudamat.less_than_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
    ...
    def greater_than_eq(self, val, target=None):
        ...
        if isinstance(val, (int, float)):
            err_code = _cudamat.greater_than_eq_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            err_code = _cudamat.greater_than_eq(
                self.p_mat, val.p_mat, target.p_mat)
    ...
    def greater_than(self, val, target=None):
        ...
        if isinstance(val, (int, float)):
            err_code = _cudamat.greater_than_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            err_code = \
                _cudamat.greater_than(self.p_mat, val.p_mat, target.p_mat)
    ...
    def upper_bound(self, val, target=None):
        """
        Perform the operation target = (self > val) ? val:self, where val can
        be a matrix or a scalar.
        """
        if not target:
            target = self

        if isinstance(val, (int, float)):
            err_code = _cudamat.upper_bound_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            err_code = \
                _cudamat.upper_bound(self.p_mat, val.p_mat, target.p_mat)
    ...
    def lower_bound(self, val, target=None):
        ...
        if isinstance(val, (int, float)):
            err_code = _cudamat.lower_bound_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            err_code = \
                _cudamat.lower_bound(self.p_mat, val.p_mat, target.p_mat)
    ...
    def cumsum(self, axis, temp=None, target=None):
        ...
        """
            ...
            if not target:
                target = empty((m, 1))
        """
        err_code = _cudamat.cumsum_by_axis(
    ...
    def choose_max_and_accumulate(self, acc):
        """
        Find the maximum value along the given dimension, where 0 represents
        the leading dimension and 1 represents the non-leading dimension.
        If a target is not prvided, a new vector is created for storing
        the result.
        """
    ...
    def choose_max(self, axis, target=None):
        """Set the argmax along axis to 1 and rest to zero."""
        ...
        err_code = _cudamat.choose_max_by_axis(
    ...
    def max(self, axis, target=None):
        ...
        err_code = \
            _cudamat.max_by_axis(self.p_mat, target.p_mat, ct.c_int(axis))
        if err_code:
            raise generate_exception(err_code)
    ...
    def argmax(self, axis, target=None):
        """
        Find the index with the maximum value along the given dimension,
        where 0 represents the leading dimension and 1 represents the
        non-leading dimension. If a target is not prvided, a new vector is
        ...
        if axis == 0:
            ...
            if not target:
                target = empty((m, 1))
        err_code = _cudamat.argmax_by_axis(
            self.p_mat, target.p_mat, ct.c_int(axis))
    ...
    def add_sqsums(self, mat, axis, mult=1.):
        """
        Add the sum of squares of mat along the given dimension to self.
        0 represents the leading dimension and 1 represents the non-leading
        dimension.
        """
        ...
        if axis == 0:
            assert self.shape == (1, n), \
                'Self has shape %s but mat has shape %s' % \
                (self.shape, mat.shape)
        elif axis == 1:
            assert self.shape == (m, 1)

        err_code = _cudamat.sqsum_by_axis(
            mat.p_mat, self.p_mat, ct.c_int(axis), ct.c_float(mult),
            ct.c_float(1.0))
        if err_code:
            raise generate_exception(err_code)
    ...
    def sqsum(self, axis, target=None):
        """
        Find the sum of squares along the given dimension, where 0 represents
        the leading dimension and 1 represents the non-leading dimension.
        If a target is not prvided, a new vector is created for storing the
        result.
        """
        ...
        err_code = _cudamat.sqsum_by_axis(
            self.p_mat, target.p_mat, ct.c_int(axis), 1.0, 0.0)
        if err_code:
            raise generate_exception(err_code)
    ...
    def norm_limit(self, norm, axis, target=None):
        ...
        if not target:
            target = self
        err_code = _cudamat.normlimit_by_axis(
            self.p_mat, target.p_mat, ct.c_int(axis), ct.c_float(norm))
        if err_code:
            raise generate_exception(err_code)
    ...
    def add_dot(self, m1, m2, mult=1.0):
        ...
        err_code = _cudamat.dot(
            m1.p_mat, m2.p_mat, self.p_mat, ct.c_float(1.), ct.c_float(mult))
        if err_code:
            raise generate_exception(err_code)
    ...
    def subtract_dot(self, m1, m2):
        ...
        err_code = _cudamat.dot(
            m1.p_mat, m2.p_mat, self.p_mat, ct.c_float(1.), ct.c_float(-1.))
        if err_code:
            raise generate_exception(err_code)
    ...
    def add_mult_sign(self, mat2, mult=1.):
        ...
        err_code = \
            _cudamat.add_mult_sign(self.p_mat, mat2.p_mat, ct.c_float(mult))
        if err_code:
            raise generate_exception(err_code)
    ...
    def subtract_mult(self, mat2, mult=1.):
        ...
        err_code = \
            _cudamat.add_mult(self.p_mat, mat2.p_mat, ct.c_float(-1. * mult))
        if err_code:
            raise generate_exception(err_code)
    ...
    def add(self, val, target=None):
        ...
        if isinstance(val, CUDAMatrix):
            err_code = _cudamat.add_elementwise(
                self.p_mat, val.p_mat, target.p_mat)
        elif isinstance(val, (int, float)):
            err_code = _cudamat.add_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix, int or float.")
    ...
    def accumulate_columns(self, indices, target, mult=1.0, avg=False):
        ...
        if avg:
            ...
            avgg = 0
        err_code = _cudamat.accumulate_columns(
            self.p_mat, indices.p_mat, target.p_mat, ct.c_float(mult),
            ct.c_int(avgg))
        if err_code:
            raise generate_exception(err_code)
        return target
    ...
    def expand_and_add(self, val, expansion_indices, target=None, mult=1.0):
        ...
        if isinstance(val, CUDAMatrix) and \
                isinstance(expansion_indices, CUDAMatrix):
            err_code = _cudamat.expand_and_add(
                self.p_mat, val.p_mat, expansion_indices.p_mat, target.p_mat,
                ct.c_float(mult))
        else:
            raise ValueError("Value must be of type CUDAMatrix, int or float.")
    ...
    def subtract(self, val, target=None):
        """Subtract val from self, where val can be a scalar or a CUDAMatrix
        with the same dimensions as self. """

        if not target:
            target = self
        if isinstance(val, CUDAMatrix):
            err_code = _cudamat.subtract_elementwise(
                self.p_mat, val.p_mat, target.p_mat)
        elif isinstance(val, (int, float)):
            err_code = _cudamat.add_scalar(
                self.p_mat, ct.c_float(-1*val), target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix, int or float.")
        if err_code:
            raise generate_exception(err_code)
    ...
    def divide(self, val, target=None):
        """Divide self by val, where val can be a scalar or a CUDAMatrix with
        the same dimensions as self. """

        if not target:
            target = self
        if isinstance(val, CUDAMatrix):
            err_code = _cudamat.divide_elementwise(
                self.p_mat, val.p_mat, target.p_mat)
        elif isinstance(val, (int, float)):
            err_code = _cudamat.divide_by_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix, int or float.")
        if err_code:
    ...
    def mult(self, val, target=None):
        ...
        if isinstance(val, CUDAMatrix):
            err_code = \
                _cudamat.mult_elementwise(self.p_mat, val.p_mat, target.p_mat)
        elif isinstance(val, (int, float)):
            err_code = _cudamat.mult_by_scalar(
                self.p_mat, ct.c_float(val), target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix, int or float.")
        if err_code:
    ...
    def apply_cos_deriv(self, val, target=None):
        ...
        if isinstance(val, CUDAMatrix):
            err_code = \
                _cudamat.apply_cos_deriv(self.p_mat, val.p_mat, target.p_mat)
        else:
    ...
    def apply_sin_deriv(self, val, target=None):
        ...
        if isinstance(val, CUDAMatrix):
            err_code = \
                _cudamat.apply_sin_deriv(self.p_mat, val.p_mat, target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix.")
        if err_code:
            raise generate_exception(err_code)
    ...
    def get_softmax_correct(self, labels, target):
        ...
        if isinstance(labels, CUDAMatrix):
            err_code = _cudamat.get_softmax_correct(
                self.p_mat, labels.p_mat, target.p_mat)
        else:
    ...
    def get_softmax_cross_entropy(self, labels, target, tiny=1e-10):
        ...
        assert target.shape == labels.shape
        if isinstance(labels, CUDAMatrix):
            err_code = _cudamat.get_softmax_cross_entropy(
                self.p_mat, labels.p_mat, target.p_mat, ct.c_float(tiny))
        else:
            raise ValueError("labels must be of type CUDAMatrix.")
        if err_code:
            raise generate_exception(err_code)
    ...
    def apply_softmax_grad(self, labels, target=None):
        ...
        if isinstance(labels, CUDAMatrix):
            if labels.shape == (1, self.shape[1]):
                err_code = _cudamat.apply_softmax_grad(
                    self.p_mat, labels.p_mat, target.p_mat)
            elif labels.shape == self.shape:
                err_code = _cudamat.subtract_elementwise(
                    self.p_mat, labels.p_mat, target.p_mat)
            else:
                raise ValueError("wrong shape of labels matrix.")
        else:
            raise ValueError("labels must be of type CUDAMatrix.")
        if err_code:
            raise generate_exception(err_code)
    ...
    def apply_logistic_deriv(self, val, target=None):
        ...
        if not target:
            target = self
        if isinstance(val, CUDAMatrix):
            err_code = _cudamat.apply_logistic_deriv(
                self.p_mat, val.p_mat, target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix.")
        if err_code:
    ...
    def apply_tanh_deriv(self, val, target=None):
        ...
        if isinstance(val, CUDAMatrix):
            err_code = _cudamat.apply_tanh_deriv(
                self.p_mat, val.p_mat, target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix.")
    ...
    def apply_rectified_linear_deriv(self, val, target=None):
        """
        Apply rectified linear derivative, where val is the activation of the
        units.
        """
        ...
        if isinstance(val, CUDAMatrix):
            err_code = _cudamat.apply_rectified_linear_deriv(
                self.p_mat, val.p_mat, target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix.")
    ...
    def apply_rectified_linear_smooth_deriv(self, val, target=None):
        """
        Apply rectified linear smooth derivative, where val is the activation
        of the units.
        """
        ...
        if isinstance(val, CUDAMatrix):
            err_code = _cudamat.apply_rectified_linear_smooth_deriv(
                self.p_mat, val.p_mat, target.p_mat)
        else:
            raise ValueError("Value must be of type CUDAMatrix.")
    ...
    def select_columns(self, indices, target):
        ...
        exception (because the programmer was lazy).
        Instead, they result in NaN values in <target>.
        """
    ...
    def swap_columns(self, indices1, indices2, target):
        ...
        <indices1> and <indices2> must be row vectors of equal length.
        Its elements are float32's representing integers, e.g. "34.0" means
        the integer "34".
        after this call, for all r,c, target[r,indices2[c]=self[r,indices1[c]].
        self can be same as target, but then the result will be
        non-deterministic if there is overlap between indices1 and indices2.
        Can be used for in-place shuffling by making sure indices1 and indices2
        do not overlap.
        This returns target.
        Negative indices are interpreted in the usual Python way: all elements
        of <indices> had better be in the range
    ...
    def set_selected_columns(self, indices, source):
        ...
        [-self.shape[1], self.shape[1]-1].
        This does bounds checking, but out of bounds indices do not raise an
        exception (because the programmer was lazy). Instead, they result in
        NaN values in <self>.
        """

        err_code = _cudamat.setSelectedRows(
...
def sum(mat, axis, target=None):
    ...
    err_code = _cudamat.dot(
        left.p_mat, right.p_mat, target.p_mat, ct.c_float(0.), ct.c_float(1.))
    if err_code:
        raise generate_exception(err_code)
...
def dot(m1, m2, mult=1.0, target=None):
    ...
    err_code = _cudamat.dot(
        m1.p_mat, m2.p_mat, target.p_mat, ct.c_float(0.), ct.c_float(mult))
    if err_code:
        raise generate_exception(err_code)
...
def log_1_plus_exp(mat, target=None, exact=False):
    ...
    If exact is True, use slow and accurate log and exp.
    """
...
def cross_entropy_bernoulli(mat, p, target=None, tiny=1e-10):
    ...
    if isinstance(p, CUDAMatrix):
        err_code = _cudamat.compute_cross_entropy_bernoulli(
            mat.p_mat, p.p_mat, target.p_mat, ct.c_float(tiny))
    else:
        raise ValueError("Value must be of type CUDAMatrix.")
...
def cross_entropy(mat, p, target=None, tiny=1e-10):
    ...
    if isinstance(p, CUDAMatrix):
        err_code = _cudamat.compute_cross_entropy(
            mat.p_mat, p.p_mat, target.p_mat, ct.c_float(tiny))
    else:
        raise ValueError("Value must be of type CUDAMatrix.")
...
def correct_preds(mat, p, target=None, cutoff=0.5):
    ...
    if isinstance(p, CUDAMatrix):
        err_code = _cudamat.correct_preds(
            mat.p_mat, p.p_mat, target.p_mat, ct.c_float(cutoff))
    else:
        raise ValueError("Value must be of type CUDAMatrix.")
...
def reformat(array):
    """Return array as a float32 array in FORTRAN order."""

    if array.dtype != np.float32 or not np.isfortran(array):
...
def cuda_set_device(dev_id):
    """Select the CUDA device with the given ID."""
...
def cublas_init():
    ...
    CUDAMatrix.ones = \
        CUDAMatrix(np.ones((MAX_ONES, 1), dtype=np.float32, order="F"))
