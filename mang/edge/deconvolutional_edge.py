...
class DeconvolutionalEdge(Edge):
    ...
    def __init__(self, nodes, conn, option):
        ...
        # derived property
        self.n_locs = (node2.shape[0] + 2 * self.padding - self.filter_size) /\
            self.stride + 1
        self.n_channel = 1 if len(node2.shape) == 2 else node2.shape[-1]
        self.n_filter = 1 if len(node1.shape) == 2 else node1.shape[-1]
        self.shape = \
    ...
    def up(self, nodes):
        ...
        cm_conv.convDown(node1.y, self.W, node2.y, self.n_locs, self.padding,
                         self.stride, self.filter_size, node2.shape[0],
                         self.n_channel)
    ...
    def gradient(self, nodes, gW):
        (node1, node2) = (nodes[self.conn[0]], nodes[self.conn[1]])
        cm_conv.convOutp(
            node2.dy, node1.y, self.W_tmp, self.n_locs, self.padding,
